...
function createHomepageGoogleMap(_latitude,_longitude,json){
    ...
    for (var i = 0; i < json.data.length; i++) {
        ...
        infoboxOptions = {
            ...
            pixelOffset: new google.maps.Size(-18, -20),
            zIndex: null,
            alignBottom: true,
            boxClass: "infobox",
...
function animateMarkers(json,newMarkers){
    if( mapLoaded == false ){
        $.each( json.data, function (i) {
            setTimeout(function(){
                if( !newMarkers[i].content.className ){
        ...
        for( var a = 0; a < json.data.length; a++ ){
            newMarkers[a].content.className = 'marker-loaded';
        }
    }
}
...
function drawClusterer(map, newMarkers){
    var clusterStyles = [
        {
            url: 'assets/img/cluster.png',
            height: 34,
